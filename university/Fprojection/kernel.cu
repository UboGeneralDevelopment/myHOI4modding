...
int main(int argc, char** argv) {//argcとかには起動時に渡す変数がはいる。
	...
	線源とディテクタ距離の指定がめんどいので、平行投影を作る。
	基本的に、入力トモグラフの1ボクセルを単位距離とし、トモグラフのはじのボクセルを座標中心とする座標で操作を行う。座標はフロート
	まず、トモグラフをもとに、トモグラフボックスの中心点を中心とするトモグラフを包み込む大きさの球形の直径を求める。
	直径サイズの投影像キャンバスを作成し、大きさをボクセルサイズの整数倍に丸める
	投影像キャンバスの各ピクセルに、ボクセルにぶつかりに行く光線を設定する。
	ここから先はGPUでやりたい。
	投影角度の指定にもとづき、キャンバス上の各光線の出発座標位置を決定する。これはフロートでよい。
	光線のステップサイズとマーチング距離を決定する。ステップ距離は変えられるがとりあえずボクセルサイズ、距離は球体の直径。
	1ステップごとに、光線が進む距離をボクセルサイズ換算でxyzに対して求める。これはフロートでよい。
	マーチングする。ステップごとに点の座標をフロートで出し、整数に丸め、座標軸上での点のボクセル位置をだす。
	そしてボクセルボックス内でのみ、トモグラフの値を取得してキャンバスに足していく。
	終了したら出力する
	*/
