...
int main(int argc, char** argv) {
	...
	*/
	...
	char in_path_name[1000], out_name[1000];
	//入力ファイルと出力ファイル名
	int v_in[3], v_out[3];
	...
	//変換前ボリューム読み込む
	if ((in_volume = fopen(in_path_name, "rb")) == NULL) {
	...
	char name[1000];
	sprintf(name, "%s-transformed-(%dx%dx%d).raw", out_name, v_out[0], v_out[1], v_out[2]);
	printf("%s", name);
